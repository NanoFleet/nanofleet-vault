...
export function createSecret(name: string, value: string, description: string | null, agentIds: string[]): SecretRow {
  const db = getDb();
  const id = randomUUID();
  const now = Date.now();
  ...
  for (const agentId of agentIds) {
    db.run('INSERT INTO secret_grants (secret_id, agent_id) VALUES (?, ?)', [id, agentId]);
  }

  return { id, name, description: description ?? null, encrypted_value, created_at: now, updated_at: now };
}

export function listSecrets(): Array<SecretRow & { agentIds: string[] }> {
  const rows = getDb().query('SELECT * FROM secrets ORDER BY name ASC').all() as SecretRow[];
  return rows.map((row) => ({
    ...row,
...
export function getSecretById(id: string): SecretRow | null {
  return (getDb().query('SELECT * FROM secrets WHERE id = ?').get(id) as SecretRow) ?? null;
}

export function getSecretByName(name: string): SecretRow | null {
  return (getDb().query('SELECT * FROM secrets WHERE name = ?').get(name) as SecretRow) ?? null;
}

export function updateSecret(
  id: string,
  fields: { name?: string; description?: string | null; value?: string; agentIds?: string[] }
): boolean {
  const db = getDb();
  ...
  const encrypted_value = fields.value !== undefined ? encrypt(fields.value) : existing.encrypted_value;
