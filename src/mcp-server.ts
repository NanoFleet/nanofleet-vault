...
import { z } from 'zod';
import { getSecretByName, isAgentAuthorized, getDecryptedValue } from './db';
...
export function createMcpServer(): McpServer {
  ...
  server.tool(
    ...
    async ({ name }) => {
      ...
      if (!secret) {
        return {
          content: [{ type: 'text', text: JSON.stringify({ error: `Secret "${name}" not found` }) }],
          isError: true,
        };
      }
      ...
      if (!isAgentAuthorized(secret.id, agentId)) {
        return {
          content: [{ type: 'text', text: JSON.stringify({ error: 'You are not authorized to access this secret' }) }],
          isError: true,
        };
      }
      ...
      return {
        content: [{ type: 'text', text: JSON.stringify({ name: secret.name, value }) }],
      };
    }
  );
